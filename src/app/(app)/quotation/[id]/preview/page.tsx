...
export default function PreviewPage() {
  ...
  const grandTotal = quotation.rooms.reduce<number>(
    (s, r) => s + r.items.reduce<number>((rs, i) => rs + i.quantity * Number(i.unitPrice), 0),
    0
  );
        ...
        >
          ...
          <section className="px-6 md:px-8 py-6 md:py-7 space-y-7">
            ...
            {summaryRooms.map((room) => {
                const roomName = room.customName ?? room.roomType?.name ?? "Room";
                const roomTotal = room.items.reduce<number>((s, i) => s + i.quantity * Number(i.unitPrice), 0);
                return (
